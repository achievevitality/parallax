...
### Draft 7 — for the systems-thinkers ecology
...
The mechanism, stated as our current best working model rather than settled fact: **the accounting boundary is almost always smaller than the causal boundary.** A sprint retro tracks the sprint. A quarterly report tracks the quarter. A success metric tracks the stated mission, not its full downstream effects. When the boundary being measured is narrower than the boundary the consequences actually reach, the exported cost is structurally invisible from inside the frame doing the measuring — not hidden through bad faith, just genuinely outside the instrument's resolution. This is why competent, well-intentioned teams and well-designed systems reliably produce this pattern anyway. The failure isn't character. It's instrumentation. This specific mechanism has held up consistently in the clinical work it was first tested against; whether it holds with the same precision at organizational or civilizational scale is one of the open questions below, not something we're claiming to have already confirmed.

## The corrigibility distinction

A system can have excellent feedback channels and still be functionally uncorrectable, if accepting correction costs the corrector something real — status, credibility, time, political capital. **A correction channel and an affordable correction are different objects.** Most organizational-improvement effort targets the channel (better dashboards, more retros, clearer reporting lines) while leaving the cost of using it untouched — which is why so much of that effort quietly stops working under real pressure, exactly when it matters most. This is the single most consistently corroborated piece of this framework so far — it's what one experienced systems-oriented reader, on first contact with an early draft of this material, reacted to most strongly and immediately, unprompted. That's a real, if small, data point in its favor. It's still one data point, not a proof.
...
The rigor behind these models isn't just asserted — it shows up in specific, checkable places: an explicit self-falsification protocol built into the earliest formal version of this framework, requiring any new hypothesis to be tested in a domain it wasn't derived from before being trusted; documented instances of deliberately declining to flatten an ambiguous pattern into a premature explanation before enough evidence existed; and, over roughly two weeks of active testing across both AI and human review — pointing these models at genuinely different domains, cross-checking findings between independent AI instances specifically to catch errors either one would miss alone, and taking real, sometimes uncomfortable correction from outside readers rather than only validation. So far, that testing has refined and sharpened the models considerably without surfacing anything that discredits their broader applicability — though it's worth being precise about what that does and doesn't establish: two weeks of testing, much of it in domains chosen because they seemed promising rather than selected adversarially, is real but early evidence. It supports continuing to test this seriously. It doesn't yet constitute confirmation that the pattern is universal.
...
*Seventh draft. Built to be pressure-tested, including by people who don't already agree with the frame — that's the actual point of sending it out.*
